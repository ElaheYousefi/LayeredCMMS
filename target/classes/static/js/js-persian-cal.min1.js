...
AMIB.persianCalendar.prototype.createDateTable=function(b,a,c){var d=cE("table"),e,g,f,h,k,j;g=this.getOption("defaultDate");
    // j=new Date;
    // f=gregorian_to_jd(j.getFullYear(), j.getMonth()+1, j.getDate());
    j = new Date();
    console.log("year="+j.getFullYear());
    console.log("month="+j.getMonth());
    console.log("day="+j.getDate() + 1);
    f = gregorian_to_jd(
        j.getFullYear(),
        j.getMonth() + 1,
        j.getDate()
    ...
    j=jd_to_persian(f);(a=isPersianDate(a))?a=persian_to_jd(a[0],a[1],a[2]):g?(a=persian_to_jd(g[0],g[1],g[2]),c=g.join("/")):a=f;c=isPersianDate(c);
    a=jd_to_persian(a);
    var l=Math.floor(persian_to_jd(a[0],a[1],1)+4)%7;e=d.insertRow(-1);g=e.insertCell(-1);g.className="pickerHead";g.colSpan=7;e=cE("div");e.className=
...
prvDef(b)};
function isPersianDate(b){var a;if(0==b.search(/^\d+\-\d+\-\d+$/))a=b.split("-",3);else if(0==b.search(/^\d+\/\d+\/\d+$/))a=b.split("/",3);else if(0==b.search(/^\d{6}|\d{8}$/))a=[b.substr(0,b.length-4),b.substr(b.length-4,2),b.substr(b.length-2,2)];else if((a=b.match(/^(\d{2})(\d{2})$/))||(a=b.match(/^(\d{1,2})[/\-](\d{1,2})$/))||(a=b.match(/^(\d{1,2})$/)))
    b=new Date,
        b=gregorian_to_jd(b.getUTCFullYear(), b.getUTCMonth()+1, b.getUTCDate()
),b=jd_to_persian(b),
    a[0]=b[0],"undefined"==typeof a[2]&&(a[2]=b[1]),12>=
...
function longYear(b){var a,c;if(100<=b)return b;a=new Date;
    a=gregorian_to_jd(b.getUTCFullYear(), b.getUTCMonth()+1, b.getUTCDate());a=jd_to_persian(a);c=100*Math.floor(a[0]/100);70<Math.abs(a[0]-c-b)&&(c+=100);return c+b}var GREGORIAN_EPOCH=1721425.5,PERSIAN_EPOCH=1948320.5;if(!fdef(mod))var mod=function(b,a){return b-a*Math.floor(b/a)};function leap_persian(b){return 682>682*((b-(0<b?474:473))%2820+512)%2816}
function jd_to_persian(b){
        var a,c,d;
        b=Math.floor(b)+0.5;
        // b=Math.floor(b)- 0.5;
        c=b-2121445.5;a=Math.floor(c/1029983);
        d=mod(c,1029983);1029982==d?c=2820:(c=Math.floor(d/366),
        d=mod(d,366),c=Math.floor((2134*c+2816*d+2815)/1028522)+c+1);
        a=c+2820*a+474;0>=a&&a--;c=b-persian_to_jd(a,1,1)+1;
        c=186>=c?Math.ceil(c/31):Math.ceil((c-6)/30);b=b-persian_to_jd(a,c,1)+1;
        return[a,c,b]
    }
...
function gregorian_to_jd(b,a,c){
    // return GREGORIAN_EPOCH-1+365*(b-1)+Math.floor((b-1)/4)+-Math.floor((b-1)/100)+
    //     Math.floor((b-1)/400)+
    //     Math.floor((367*a-362)/12+(2>=a?0:leap_gregorian(b)?-1:-2)+c)
    console.log("month="+ a);
...
function persianMonthDays(b,a){return 6>=a?31:11>=a?30:12==a?leap_persian(b)?30:29:0}if(!fdef(stopPropg))var stopPropg=function(b){b.stopPropagation?b.stopPropagation():b.cancelBubble=!0};if(!fdef(prvDef))var prvDef=function(b){b.preventDefault?b.preventDefault():b.returnValue=!1};
